...
interface WatchContextInterface {
...
const initialState: WatchContextInterface = {
...
const reducer = (state: WatchContextInterface, action: ACTIONTYPE) => {
  switch (action.type) {
    case ADDTOWATCHLIST: {
      return {
        ...state,
...
function WatchProvider({ children }: WatchProviderProps) {
  ...
  return (
    <WatchContext.Provider value={{ watchlist: state.watchlist, addMovieToWatchlist, removeMovieFromWatchlist }}>
      {children}
